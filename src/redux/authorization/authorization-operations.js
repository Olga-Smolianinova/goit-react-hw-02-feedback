...
// у axios  на определенный defaults можно на определенный header привязать какое-либо свойство для всех будущих http-запросов
...
const token = {
  ...
  set(token) {
    axios.defaults.headers.common.Authorization = `Bearer ${token}`; //исходя из этого привязывает свойство Bearer - это Носитель, к которому указываем, что Носителем будет наш token
  },
  unset() {
    axios.defaults.headers.common.Authorization = '';
  }, //вызов unset() снимает Носитель Bearer
};
